...
**Total tests: 187 · Test suites: 18 · Failures: 0 · Skipped: 0**
**Command: `forge test` → "Ran 18 test suites … 187 tests passed, 0 failed, 0 skipped (187 total tests)"**
...
| 4 | `CreditGateVaultGoTeeCompatTest` | `test/CreditGateVault.tee-compat.t.sol` | **2** | Cross-language EIP-191 compatibility: a signature produced by the **Go TEE handler** (`/action` response body) is accepted by the **Solidity** vault's `submitEligibility` via `ecrecover`. Proves the Go signer and the on-chain verifier use the same payload layout + EIP-191 prefix. |
...
| 13 | `CreditGateVaultProtocolReserveTest` | `test/CreditGateVault.protocol-reserve.t.sol` | **13** | Protocol reserve fund: 1% of interest-equivalent collateral accrues to a backstop reserve (Aave Safety Module pattern, Bug 2 fix — fee on interest, not full collateral), `getProtocolReserve` accounting, `withdrawReserve` owner-only withdrawal, reserve caps, zero-fee edge cases (incl. immediate-repayment which now returns the full collateral), and accounting invariants across multiple draw/repay flows. Added by subagent #89. |
...
> --summary` reports. Both framings agree on the **187-test total**.
...
13. **Protocol reserve (13)** — 1% of interest-equivalent collateral accrues to a backstop reserve (Aave Safety Module pattern, Bug 2 fix — fee is charged on the interest-equivalent FXRP, not the full deposit); `getProtocolReserve`, owner-only `withdrawReserve`, reserve caps, zero-fee edges (incl. immediate repayment which returns the full collateral), and accounting invariants across draw/repay flows.
