...
|  |  |  |  |  |
|---|---|---|---|---|
|| 🟢 **Live on Coston2** | ✅ **184 tests** | ✅ **8 invariants** | ✅ **4 Flare primitives** | ✅ **Source verified** ||
...
```bash
# 1. Contracts — 184 tests across 17 suites, 0 failures
forge test
...
- **180-test Foundry suite across 17 suites** (8 invariant/fuzz tests, cross-language Go-TEE ↔ Solidity compat, real malicious-token reentrancy attack)
...
| 5 | **184 tests / 17 suites + 8 invariant/fuzz + security audit clean** — all M1/M2/L1/L2/L4/L5 findings fixed. | `planning/security-audit/verdict.md` (PASS) + `forge test` |
...
| `test/CreditScoreSBT.t.sol` | 9 soulbound-token tests — mints on first repayment, updates on subsequent ones, non-transferable, score reflects reputation, only-vault-can-mint, getScore, tokenURI, metadata |
